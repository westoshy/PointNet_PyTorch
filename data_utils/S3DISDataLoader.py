...
class S3DISDatasetWholeScene():
    ...
    def __init__(self, root, block_points=4096, split='test', test_area=5, stride=0.5, block_size=1.0, padding=0.001):
        ...
        """
        train: test_areaで指定した番号は除いて読み込む
        test : test_areaで指定した番号だけ読み込む
        """
        if self.split == 'train':
        ...
        self.scene_points_list = []     # 点群(x,y,z,R,G,B)
        self.semantic_labels_list = []  # ラベル
        self.room_coord_min, self.room_coord_max = [], []
    ...
    def __getitem__(self, index):
        """
        オブジェクトに[index]でアクセスする
        indexで指定したデータをモデルに入力できるように指定の点数をもつ
        ブロック単位で切り出して返す
        """
...
if __name__ == '__main__':
    data_root = 'D:\dataset\S3DIS\stanford_indoor3d\\'
